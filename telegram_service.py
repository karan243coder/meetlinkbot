        ...
        if pyro_client and pyro_client.is_connected and pyro_loop:
            ...
            if file_size <= max_chunk_size:
                ...
                if is_video and file_path.endswith(".mp4"):
                    fut = asyncio.run_coroutine_threadsafe(pyro_client.send_video(chat_id=CHANNEL_ID, video=file_path, caption=caption, supports_streaming=True), pyro_loop)
                    fut.result(timeout=600)
                else:
                    fut = asyncio.run_coroutine_threadsafe(pyro_client.send_document(chat_id=CHANNEL_ID, document=file_path, caption=caption), pyro_loop)
                    fut.result(timeout=600)
            else:
                ...
                for i, part_path in enumerate(parts):
                    ...
                    fut = asyncio.run_coroutine_threadsafe(pyro_client.send_document(chat_id=CHANNEL_ID, document=part_path, caption=part_cap), pyro_loop)
                    fut.result(timeout=600)
