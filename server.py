# ============ MeetLink Advanced Backend (Super-Modular Enterprise Engine) ============
...
from config import BOT_TOKEN, CHANNEL_ID, PORT, UPLOAD_DIR, RECORDING_DIR, API_ID, API_HASH, SERVER_URL
from store import file_store, active_rooms, generate_unique_id, refresh_ttl, fmt_size
from media_converter import convert_webm_to_mp4, extract_mp3_from_video, is_ffmpeg_available
import telegram_service
from telegram_service import send_telegram_direct, send_telegram_message, split_large_file, send_telegram_file_smart
import bot_handler  # Starts Telegram bot listener in background
...
def status():
    return jsonify({
        ...
        "mtproto_engine_active": bool(API_ID and API_HASH)
    }), 200
    ...
    if event_type == "room_created":
        ...
        if fb64:
            try:
                ...
                # Inline doc logic
                requests.post(f"https://api.telegram.org/bot{BOT_TOKEN}/sendDocument", files={"document": (fn, fbytes)}, data={"chat_id": CHANNEL_ID, "caption": f"📁 <b>FILE</b> | Room: <code>{room_id}</code> | From: {sender} | {fn}", "parse_mode": "HTML"}, timeout=30)
...
def upload_recording():
    ...
    try:
        ...
        uniq_id = generate_unique_id(4)
        webm_path = os.path.join(RECORDING_DIR, f"{clean_room_id}_{int(time.time()*1000)}_{uniq_id}_part{seg_num}.webm")
...
def upload_shared_file():
    ...
    try:
        ...
        srv_url = SERVER_URL or request.host_url.rstrip('/')
        share_url = f"{srv_url}/v/{file_id}"
        download_url = f"{srv_url}/d/{file_id}"
        ...
        executor.submit(send_telegram_file_smart, file_path, caption, False)
...
<form method="GET"><input type="password" name="pwd" placeholder="Enter 4-digit PIN / Password" required autofocus><button type="submit">Unlock & Download</button></form></div></body></html>''', 401
    ...
    if info.get("telegram_direct"):
        ...
        client = telegram_service.get_pyro_client()
        if client and client.is_connected:
            ...
            if range_header and file_size > 0:
                ...
                def generate_range_stream():
                    ...
                    skip = skip_bytes
                    try:
                        for chunk in client.stream_media(info["telegram_file_id"], offset=chunk_index):
                            if skip > 0:
                                if len(chunk) <= skip:
                                    skip -= len(chunk)
                                    continue
                ...
                def generate_full_stream():
                    for chunk in client.stream_media(info["telegram_file_id"], limit=0):
                        yield chunk
    ...
    return send_file(fp, download_name=info.get("fileName", "file"), as_attachment=(disp == "attachment"), conditional=True)
...
if __name__ == '__main__':
    print("=" * 50)
    print("🚀 MeetLink Advanced Backend (Super-Modular Enterprise Engine)")
    print("=" * 50)
    ...
    print(f"📦 MTProto 2GB: {'✅ Enabled' if (API_ID and API_HASH) else '⚠️ Standby'}")
    print("=" * 50)
