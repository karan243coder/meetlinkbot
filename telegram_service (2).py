...
def get_clean_chat_id(cid):
    if isinstance(cid, str):
        c_str = cid.strip()
        if (c_str.startswith("-") and c_str[1:].isdigit()) or c_str.isdigit():
            return int(c_str)
    return cid
...
def send_telegram_file_smart(file_path, caption, is_video=False):
    ...
    try:
        ...
        clean_cid = get_clean_chat_id(CHANNEL_ID)
        
        # 1. MTProto Pyrogram Engine (2GB Limit per file)
        if pyro_client and pyro_client.is_connected and pyro_loop:
            try:
                ...
                if file_size <= max_chunk_size:
                    ...
                    if is_video and file_path.endswith(".mp4"):
                        fut = asyncio.run_coroutine_threadsafe(pyro_client.send_video(chat_id=clean_cid, video=file_path, caption=caption, supports_streaming=True), pyro_loop)
                        fut.result(timeout=600)
                    else:
                        fut = asyncio.run_coroutine_threadsafe(pyro_client.send_document(chat_id=clean_cid, document=file_path, caption=caption), pyro_loop)
                        fut.result(timeout=600)
                else:
                    ...
                    for i, part_path in enumerate(parts):
                        ...
                        fut = asyncio.run_coroutine_threadsafe(pyro_client.send_document(chat_id=clean_cid, document=part_path, caption=part_cap), pyro_loop)
                        fut.result(timeout=600)
                ...
                print(f"⚠️ [Pyrogram Upload Note] {e_pyro} -> Falling back to HTTP Bot API!")
